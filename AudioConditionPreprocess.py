...
def video_to_audio_features(csv_path, frac=(1/6), frac_id=0):
    ...
    for idx in tqdm(new_idxs):
        ...
        videos_in_id = [ele for ele in os.listdir(idx_path) if os.path.isfile(ele) and ele[-3:] == "mp4"]
        if not os.path.isfile(os.join(idx_path, videos_in_id[0][:-3] + 'npy')):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--frac', type=float,
                        default=(1/6), help='fraction to break he df into, for multi-processing')
    parser.add_argument('--frac_id', type=int,
                        default=0, help='frac_id to know which fraction to run')
    video_to_audio_features('/scratch/tan/train_landmarks.csv', frac=parser.frac, frac_id=parser.frac_id)
    video_to_audio_features('/scratch/tan/val_landmarks.csv',  frac=parser.frac, frac_id=parser.frac_id)
